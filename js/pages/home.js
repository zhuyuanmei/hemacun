...
define(function (require, exports, module) {
    ...
    var tpl = {
        ...
        degreeBarTpl: [
            '<div class="radio-item J_RadioItem" data-id="{{id}}" data-curValue="一般" style="display:none;">',
            ...
            '<span class="grade-item2 current J_CurrentGrade2">一般</span>',
            '<span class="grade-item3 J_CurrentGrade3">&nbsp;</span>',
            '</div>',
            ...
            '<span class="grade-item2 J_OtherGrade2">&nbsp;</span>',
            '<span class="grade-item3 J_OtherGrade3">非常符合</span>',
            '</div>',
            '</li>',
            '</ul>',
            '</dd>',
        ...
        videoTpl:[
            ...
            '{{#hasVideo}}',
            '<video id="J_Video{{rootId}}" poster="" preload="auto" style="width:96%;margin:2%;" src="/{{videoUrl}}">',
            '<p>Your browser does not support the video tag.</p>',
            '</video>',
            '{{/hasVideo}}',
            '{{#hasAudio}}',
            '<audio id="J_Audio{{rootId}}" src="/{{audioUrl}}">',
            ...
            '<ul>',
            '{{#answerList}}',
            '<li><a href="javascript:;" class="J_SubmitChoose" data-option-id="{{rootId}}" data-answer-id="{{id}}"><img src="/{{imageUrl}}"></a></li>',
            '{{/answerList}}',
            '</ul>',
            '</div>',
            '{{/hasAnswerId}}',
    ...
    if ($('#J_MotherInfo').length) {
        ...
        //radio交互
        $testPart1.delegate('input[type="radio"]', 'change', function () {
            var $this = $(this);
        ...
        //昵称交互
        $testPart1.delegate('#J_BabyName', 'keyup', function () {
            var $this = $(this);
        ...
        var treeArr = [];
        $testPart1.delegate('.J_TreeItem','click',function(){
            var $this = $(this);
            var $curParent = $this.parents('.J_RadioItem');
        ...
        //'滑动效果'
        $testPart2.delegate('.J_Wrap', 'touchmove', function (e) {
            var $this = $(this);
        ...
        $testPart2.delegate('.J_Wrap', 'touchend', function (e) {
            var $this = $(this),
            ...
            if (e.changedTouches[0].pageX < 80) {
                ...
                $curParent.find('.J_OtherGrade2').text('一般');
                $curParent.find('.J_OtherGrade3').text('非常符合');
            ...
            } else if (e.changedTouches[0].pageX >= 80 && e.changedTouches[0].pageX <= 210) {
                ...
                $curParent.find('.J_CurrentGrade span').text('');
                $curParent.find('.J_CurrentGrade span').removeClass('current');
                $curParent.find('.J_CurrentGrade2').text('一般');
                $curParent.find('.J_CurrentGrade2').addClass('current');
            } else if (e.changedTouches[0].pageX > 210) {
                ...
                $curParent.find('.J_CurrentGrade span').text('');
                $curParent.find('.J_CurrentGrade span').removeClass('current');
                $curParent.find('.J_CurrentGrade3').text('非常符合');
            ...
            if ($this.hasClass('nextVisible')) {
                ...
                if(parseInt(curId) === $testPart1.find('.J_RadioItem').length){
                    ...
                    if ($($('.J_RadioItem')[nextId]).attr('data-curValue') !== '') {
                        $commerFooter.css('background', '#BCEF00');
                        $commerFooter.hasClass('nextVisible');
                    } else {
                        $commerFooter.css('background', '#eee');
                        $commerFooter.removeClass('nextVisible');
                    }
